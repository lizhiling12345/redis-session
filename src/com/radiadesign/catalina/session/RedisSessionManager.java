...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
	...
	protected String sentinelHostPort ="";
	protected int maxActive; //设置最大连接数 
	protected int maxIdle; //设置最大空闲数 
	protected int maxWait;//设置超时时间 
	protected int maxResetPool;//根据这个数值，重新初始化Pool 
	protected int _currentError=0;//根据这个数值，重新初始化Pool 
	...
	protected String Pool_Type_Sentinel ="Sentinel";
	...
	public String getSentinelHostPort() {
		return sentinelHostPort;
	}

	public void setSentinelHostPort(String sentinelHostPort) {
		this.sentinelHostPort = sentinelHostPort;
	}
	...
	public int getMaxResetPool() {
		return maxResetPool;
	}

	public void setMaxResetPool(int maxResetPool) {
		this.maxResetPool = maxResetPool;
	}
	...
	protected Jedis acquireConnection() {
		...
		try {
			...
			_currentError =_currentError + 1;
		}
		
		try {
			if( _currentError>getMaxResetPool() ){
	...
	public void start() throws LifecycleException {
		for (Valve valve : getContainer().getPipeline().getValves()) {
			if (valve instanceof RedisSessionHandlerValve) {
				...
				log.info("RedisSessionManager:Attached to RedisSessionHandlerValve");
				
				if( getUserProperties()!=null ){
					System.setProperty("userProperties", getUserProperties());
					log.info("RedisSessionManager:userProperties:"+getUserProperties());
				}
		...
		log.info("RedisSessionManager:Will expire sessions after " + getMaxInactiveInterval()+ " seconds");
		initializeDatabaseConnection();
...
//			}
			...
			//------lizl7---------
			if (sessionIsDirty) {
				byte[] arrTemp =serializer.serializeFrom(redisSession);
				jedis.set(binaryId,arrTemp);
			}
	...
	private void initializeDatabaseConnection() throws LifecycleException {
		try {
	        ...
	        jedisPoolConfig.setMaxActive(getMaxActive()); //设置最大连接数 
	        jedisPoolConfig.setMaxIdle(getMaxIdle()); //设置最大空闲数 
	        jedisPoolConfig.setMaxWait(getMaxWait());//设置超时时间 
	        jedisPoolConfig.setTestOnBorrow(false); 
	        
			//---lizl---
			if( Pool_Type_Sentinel.equals( getPoolType() ) ){
				Set<String> listMaster =new HashSet<String>();
				String[] arrTemp =getSentinelHostPort().split(",");
				for(  String strTemp : arrTemp){
					listMaster.add(strTemp);
				}

	            connectionPool = new JedisSentinelPool(getSentinelMaster(), listMaster, jedisPoolConfig, getTimeout(),getPassword());
	            _currentError =0;
	            log.info("RedisSessionManager:JedisSentinelPool:"+ getPoolType()+"_"+getSentinelMaster()+"_"+getSentinelHostPort());
	            
			}else{
				connectionPool = new JedisPool(jedisPoolConfig, getHost(),getPort(), getTimeout(), getPassword());
				
				log.info("RedisSessionManager:JedisPool:" + getHost()+"_"+getTimeout());
			}
	...
	private void initializeSerializer() throws ClassNotFoundException,
		...
		log.info("RedisSessionManager:Attempting to use serializer :" + serializationStrategyClass);
